...
if __name__ == '__main__':
    ...
    if nc_mgr is None:
        ...
        #Get serial number of router 
        serial_num = ztp_script.get_serial_number()
        for key in SERIAL_NO_MAP:
            if key == serial_num:
                rtr_num = SERIAL_NO_MAP[key]
        
        #Get Corresponding router XML files 
        for i in XML_FILE_LIST:
            cmd = "wget " + SERVER_XML_URL + rtr_num + "/" + i
            if not ztp_script.run_bash(cmd)["status"]:
                ztp_script.syslogger.info("Successfully downloaded " + i)
            else:
                ztp_script.syslogger.info("Failed to download " + i)
         
        #Edit running config and commit changes of each xml file on router                                                                                 
        for j in XML_FILE_LIST:
            with open(j, 'r') as f3:
                config = f3.readlines()
                response = nc_mgr.edit_config(''.join(config), format='xml', target='candidate',  default_operation='merge')
                response1 = nc_mgr.commit()
                print(response)
